...
async def main() -> None:
    ...
    try:
        existing = await conn.fetchrow(
            "SELECT id, name FROM tenants WHERE name = $1", "dev-tenant"
        )
        if existing:
            print(f"Tenant '{existing['name']}' already exists (id={existing['id']}).")
            print("API key was printed at first creation. Reset the DB to regenerate it.")
            return
        ...
        print(f"  {raw_key}")
        print(f"{'='*60}\n")
        print(f"Ticker universe: {', '.join(ticker_universe)}")
    finally:
        await conn.close()
